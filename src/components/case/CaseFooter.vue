<template>
  <section v-if="showFooter" class="case-page-footer layout">
    <div class="layout__text" v-if="!isProgress">
      <h2>{{ title }}</h2>
      <div class="mb" v-html="text" />
      <CaseGithubLink v-if="link" :link="link" :linkText="linkText"/>
    </div>
    <div class="layout__text footer-in-progress" v-else>
      {{ $t("This case is currently in motion; don't miss out news — subscribe for updates in the box below") }}
    </div>
  </section>
</template>
...
export default {
  ...
  components: {
    CaseGithubLink: () => import('~/components/case/CaseGithubLink.vue'),
  }, 
...
<style scoped>
  ...
  .case-page-footer .footer-in-progress {
    padding-left: 72px;
    font-weight: 800;
    min-height: 50px;
    display: flex;
