...
Загалом, тест полягає у відправленні відповідних повідомлень від одного агента до іншого.

Далі Promisor знаходить новий контракт зобов'язання в мережі та починає його виконання. Після завершення роботи надсилається повідомлення з результатом, який потім фіксується в контракті.

Покрокова інструкція щодо запуску знаходиться у [README.md](https://github.com/Multi-Agent-io/neon-kuka-demo/blob/main/README.md)
...
Після того, як новий контракт Liability було виявлено на Neon devnet, агент починає роботу. Результат роботи (телеметрія) записується у файл і відправляється в IPFS, хеш з файлу зберігається як результат.
...
### Truffle не працює, якщо не отримує відповідь від мережі

Під час процесу розгортання контрактів з'ясувалося, що проксі не завжди повертає відповідь вчасно, через що Truffle завершується з помилкою:
...
Якщо додати таймаут і повторну спробу тут [index.ts#L75](https://github.com/trufflesuite/truffle/blob/develop/packages/interface-adapter/lib/adapter/web3/index.ts#L75), то деплой повинен проходити без помилок.

### Відсутній WS/WSS Endpoint
...
Незважаючи на деякі особливості роботи мережі, зрештою всі контракти, що брали участь, продемонстрували свою працездатність у середовищі Neon.