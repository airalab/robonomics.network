...
The work is in [airalab/robonomics#652](https://github.com/airalab/robonomics/pull/652), tracked in [#629](https://github.com/airalab/robonomics/issues/629):
...
- the storage left behind by the old 4.x governance pallets (Democracy, Treasury, Technical Committee), switched off but never cleaned, is being cleared before the new pallets take over the same names;
...
The plan is to ship governance in version 5.1, together with the next CPS updates, as a runtime upgrade on the Polkadot parachain. The numbers above are current code, not final law. Once governance is live, changing them will itself be a referendum.
