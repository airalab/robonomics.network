---
title: "Tests réussis de transfert de XRT entre Sepolia et Rococo Asset Hub via Snowbridge"
date: 2024-07-22
published: true
locale: 'fr'
...
Le processus d'enregistrement devrait prendre environ 30 minutes, et le nouveau token devrait apparaître dans foreignAssets dans Rococo Asset Hub. Cela peut être vérifié en effectuant une requête d'état de la chaîne dans Rococo Asset Hub (https://polkadot.js.org/apps/?rpc=wss%3A%2F%2Frococo-asset-hub-rpc.polkadot.io#/chainstate) :
...
3.  J'ai initié le transfert de XRT de Sepolia vers Rococo Asset Hub en suivant les instructions de Snowbridge à l'aide de l'exemple WETH : https://docs.snowbridge.network/rococo-testnet/rococo-sepolia-token-transfers#id-5.-send-weth-to-polkadot
    ...
    Une note importante concernant le champ `destinationAddress.data` : sa valeur devait être placée entre guillemets (par exemple "0xb0fca9a0c5e03ab5b2294784fbbabea010295b3e1e48575cfe18dc2172ef657d").
    ...
    Un total de 15 XRT a été envoyé, donc le solde dans Rococo Asset Hub était le suivant :
    ```
    {
...
https://polkadot.js.org/apps/?rpc=wss%3A%2FCette transaction peut être consultée dans l’explorateur de blocs Rococo Asset Hub :
...
Cette étape a été réalisée relativement rapidement, en moins d’une demi-heure. En conséquence, 1 XRT a été transféré de nouveau vers Sepolia. Le solde XRT dans Rococo Asset Hub est resté à 14 XRT :
