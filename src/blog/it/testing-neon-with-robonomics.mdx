...
description: "Lo scopo di questi test era validare la prontezza della piattaforma Neon per passare alla fase di produzione utilizzando gli smart contract Robonomics, che permettono ai sistemi ciber-fisici (robot) di effettuare transazioni tecnico-economiche."
abstract: ""
---

## TL;DR

Lo scopo di questi test era validare la prontezza della piattaforma [Neon](https://neon-labs.org/) per passare alla fase di produzione utilizzando gli smart contract Robonomics, che permettono ai sistemi ciber-fisici (robot) di effettuare transazioni tecnico-economiche.

Nella rete Robonomics, l'interazione viene stabilita tra due parti: il Promissario - colui che ordina il servizio, invia una descrizione del compito tecnico e trasferisce i fondi per la sua esecuzione; il Promittente - colui che esegue il servizio. È stato testato uno scenario tipico, in cui viene stipulato un contratto di responsabilità tra un Promissario e un Promittente - seguito dalla validazione del lavoro risultante da parte di una terza parte, e dal pagamento di una commissione al nodo che ha accompagnato il contratto tra le parti.
...
Nonostante alcune particolarità nel funzionamento della rete, alla fine tutti i contratti partecipanti hanno dimostrato la loro operatività nell'ambiente Neon.