...
Il passo successivo è stato inviare il token XRT da Sepolia a Rococo Asset Hub. Per questo, abbiamo eseguito le seguenti azioni:
...
2. Poi è stata richiesta la commissione necessaria per inviare il token:
...
3.  È stato avviato il trasferimento di XRT da Sepolia a Rococo Asset Hub seguendo le istruzioni di Snowbridge utilizzando l'esempio WETH: https://docs.snowbridge.network/rococo-testnet/rococo-sepolia-token-transfers#id-5.-send-weth-to-polkadot
    ...
    Sono stati inviati in totale 15 XRT, quindi il saldo in Rococo Asset Hub appariva così:
    ```
    {
...
L'ultimo passaggio è stato l'invio di XRT da Rococo Asset Hub a Sepolia. Abbiamo eseguito la seguente extrinsic in Rococo Asset Hub: 

https://polkadot.js.org/apps/?rpc=wss%3A%2FQuesta transazione può essere visualizzata nell'explorer dei blocchi di Rococo Asset Hub:
...
Questo passaggio è stato completato relativamente rapidamente, in meno di mezz'ora. Di conseguenza, 1 XRT è stato trasferito nuovamente su Sepolia. Il saldo XRT su Rococo Asset Hub è rimasto a 14 XRT:
