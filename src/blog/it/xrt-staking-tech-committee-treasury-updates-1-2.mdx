...
Il pagamento per transazione è autoesplicativo. La funzione di abbonamento necessita di qualche spiegazione. Alcuni casi d'uso IoT necessitano di accedere periodicamente a funzioni cloud, ed è altamente indesiderabile che questi accessi falliscano. Il meccanismo del gas-price non risolve ancora completamente questo problema, soprattutto quando si tratta di garantire transazioni su ogni blocco. Tenendo presente ciò, abbiamo avuto l'idea di utilizzare il meccanismo di staking per gestire una quota garantita con un modello di abbonamento. Sul lato Substrate, abbiamo fatto un piccolo trucco, basato sull'elaborazione prioritaria delle transazioni a peso zero. Se un dispositivo IoT attiva un abbonamento, tutte le sue transazioni avranno priorità su tutte le altre, tranne ovviamente le altre transazioni a peso zero. Man mano che più token entrano nel pool di staking della rete, più abbonamenti saranno disponibili per gli utenti.
...
A partire dal [blocco n. 235.047](https://robonomics.subscan.io/block/235047), abbiamo circa mille account; il maggior detentore di token è il tesoro. I fondi del tesoro non parteciperanno allo staking.
