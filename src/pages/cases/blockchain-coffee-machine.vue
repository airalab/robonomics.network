<template>
  ...
  <Header
    ...
    <section class="case-page layout">
      <section class="case-page__content hyphens">
        ...
        >
          ...
          <p><b>{{ $t('Step 1: automated coffee bean ordering') }}</b>.{{ $t(`We've made an arrangement with the barista, Nikita, who's located near the office. He'll supply us with coffee beans upon receiving an alert from the bot. The bot sends a message when the coffee is running low, specifying the quantity of coffee (in kilograms) needed for the order and the corresponding payment amount determined by the team. With these details, Nikita can select the coffee according to his preference.`) }} </p>

          <p><b>{{$t('Step 2: automatic access to smart office.')}}</b>{{$t(`Simultaneously with the order, the coffee machine generates a QR code. Nikita then presents this QR code to the camera for office entry.`)}}</p>

          <p><b>{{$t('Step 3: distribution of coffee tokens to the team.')}}</b>{{$t(`Once the local camera scans the QR code, the smart home management system initiates a payment to Nikita and distributes new coffee tokens among the team members.`)}}</p>

          <p><b>{{$t('Step 4: drink coffee!')}}</b>{{$t('In order to receive a fresh cup of coffee, a team member needs to send a coffee token to the address of the coffee machine.')}}</p>
          
          <h3 class="youtube-icon">{{$t('Coffee beans delivery process:')}}</h3>
    ...
    <Footer 
      :title="$t('DIY or how to build your own smart blockchain coffee machine')"
      :text="`<p>${$t('You can find a detailed tutorial and technical specifications in the open-source GitHub repository. Keep in mind that you can use any coffee machine you prefer and almost any token of your choice to bring this project to life in your own space.')}</p>`"
